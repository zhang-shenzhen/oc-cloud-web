<template>
  <div ref="page" class="d2-theme-header-menu" :class="{'is-scrollable': isScroll}" flex="cross:center">
    <div ref="content" class="d2-theme-header-menu__content" flex-box="1" flex>
      <div ref="scroll" class="d2-theme-header-menu__scroll" flex-box="0" :style="'transform: translateX(' + currentTranslateX + 'px);'">
        <el-menu mode="horizontal" :default-active="active" @select="handleMenuSelect">
          <template v-for="(menu, menuIndex) in header">
            <d2-layout-header-aside-menu-item v-if="menu.children === undefined" :key="menuIndex" :menu="menu" />
            <d2-layout-header-aside-menu-sub v-else :key="menuIndex" :menu="menu" />
          </template>
        </el-menu>
      </div>
    </div>
    <div v-if="isScroll" class="d2-theme-header-menu__prev" flex-box="0" flex="main:center cross:center" @click="scroll('left')">
      <i class="el-icon-arrow-left" />
    </div>
    <div v-if="isScroll" class="d2-theme-header-menu__next" flex-box="0" flex="cross:center" @click="scroll('right')">
