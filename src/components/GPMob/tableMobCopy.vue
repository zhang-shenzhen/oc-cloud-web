<template>
  <div class="tableMob">
    <el-table ref="multipleTable"
      ...
      <el-table-column v-if="tableConfig.status.isAllSet"
                       type="selection"
                       width="55" />
      <el-table-column v-for="item in tabHeadConfig_1"
                       :key="item.index"
                       :label="item.title"
        ...
        <template slot-scope="scope">
          <span :class="'v-'+item['key']">
            <slot v-if="item.key != 'quantity' && item.key != 'enabled'">
              <template>{{ scope.row[item['key']] }}</template>
            </slot>
            ...
            <slot v-if="item.key == 'enabled'">
              <template>
                <span @click="integralThis(scope.row, {name: 'enabled',btn: '状态'})">
                  <el-switch :value="scope.row.enabled"
                             active-color="#13ce66"
      ...
      <!-- 操作 -->
      <el-table-column v-for="item in tabHeadConfig_2"
                       :key="item.index"
                       :label="item.title"
                       :width="item.width"
                       fixed="right">
        <template slot-scope="scope">
          <span :class="'v-'+item['key']">
            <template>
              <el-button v-for="(val,index) in tableConfig.operating"
                         :key="index"
                         :style="val.style"
                         v-if="val.show == undefined || val.show"
                         type="text"
                         size="mini"
                         @click="integralThis(scope.row,val)">{{ $t(val.name) }}</el-button>
            </template>
          </span>
        </template>
      </el-table-column>
...
export default {
  ...
  created () {
    ...
    this.tableConfig.tabHeadConfig.map((val, index) => {
      if (language == 'zh-CN') {
        val.title = val['cn-name']
      } else {
        val.title = val['en-name']
      }
      val.index = index
      if (val.key == 'operating') {
        this.tabHeadConfig_2.push(val)
      } else {
        this.tabHeadConfig_1.push(val)
      }
    })
  },
  mounted () {
