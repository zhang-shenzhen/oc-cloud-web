<template>
  <div class="tableMob">
    <el-table
      ref="multipleTable"
      class="userTab"
      :highlight-current-row="tableConfig.status.tableConfigHighlightCurrentRow"
      row-key="id"
      :data="tableData"
...
export default {
  ...
  created() {
    ...
    this.tableConfig.tabHeadConfig.map(val => {
      if (language == 'zh-CN') {
    ...
    if (
      this.tableConfig.status.tableConfigHighlightCurrentRow === null ||
      this.tableConfig.status.tableConfigHighlightCurrentRow === undefined
    ) {
      this.tableConfig.status.tableConfigHighlightCurrentRow = true
    }
    // 对tabSot管理
  ...
  methods: {
    ...
    // 对 列表的排序管理
    _initDefaultSort() {
      const defaultSort = this.tableConfig.defaultSort || {
        prop: 'id',
        order: 'descending'
      }
