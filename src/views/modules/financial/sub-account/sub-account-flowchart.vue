    ...
    <el-table :data="tableData"
      ...
      <el-table-column :label="$t('handle')">
        <template slot-scope="scope">
          <el-button size="mini"
                     type="text"
                     v-if="scope.row.status==1 && scope.row.statusType==2 && isShowId"
                     @click="handleEdit(scope.$index, scope.row)">转 账</el-button>
          <el-button size="mini"
                     type="text"
...
<script>
import { ListLedger, CollectingInformation, ReviseLedgerRecord } from "@/api/financial/index";
...
export default {
  ...
  created () {
    ...
    this.isShowId = this.info.storeId // 为空的时候隐藏转账按钮
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      this.$confirm("确定此操作？", "提示", {
        ...
        .then(() => {
          ReviseLedgerRecord({ id: row.id, status: 2 }).then(res => {
            if (res.code == 0) {
              this.$notify({
                title: '成功',
                message: '转账成功',
