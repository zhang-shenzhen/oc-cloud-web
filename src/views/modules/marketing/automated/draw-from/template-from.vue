<template>
  <div>
    ...
    >
      ...
      <el-form-item label="推送模板消息">
          <el-select size="small" :disabled="action == 'look'" style="width:100%;max-width:570px;" v-model="templateId" placeholder="选择模板">
              <el-option v-for="(message,index) in templates" :key="index" :label="message.templateTitle" :value="message.id"></el-option>
            </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.operation.operationItems[0].type = 11;
      this.operation.operationItems[0].value =[{
          id:this.templateId
      }];
      ...
      this.templates.map(val =>{
       if(this.templateId == val.id){
          this.operation.showEleText = val.templateTitle;
       }
     })
