<template>
  <d2-container class="page">
    <h3 class="list-title" style="margin-top:-10px;">
      {{ $t('brand.CheckIn') }}
    </h3>
    <el-form
      ref="ruleForm"
    ...
    >
      ...
      <el-form-item label="Title" prop="title">
        <el-col :span="18">
          <el-input v-model="ruleForm.international.title" clearable />
        </el-col>
      </el-form-item>
      ...
      <el-form-item label="Check in Desc" prop="description">
        <el-col :span="18">
          <Tinymce
            id="tinymce2"
            v-model="ruleForm.international.description"
            :quan-tity="60"
            :height="220"
          />
        </el-col>
      </el-form-item>
      <el-form-item :label="$t('brand.getIntegral')" prop="num">
        <el-col :span="10">
          <el-input
            ...
            min="0"
            :placeholder="
              $t('p') + $t('integral') + '(' + $t('PositiveInteger') + ')'
            "
            clearable
          />
        </el-col>
      </el-form-item>
      <el-form-item :label="$t('brand.CheckInTime')" prop="date">
        <el-col :span="10">
          <el-date-picker
      ...
      <el-form-item :label="$t('brand.ContinuousSignInReward')">
        <el-col :span="10">
          <el-checkbox
            v-model="subInfo.openContinuousAttendance"
            @change="isreward"
            >{{ $t('open') }}</el-checkbox
          >
        </el-col>
      ...
      <el-form-item v-if="subInfo.openContinuousAttendance">
        <el-col :span="13">
          <div
            v-for="(item, index) in subInfo.continuousAttendanceAward"
            :key="index"
            class="box"
          >
            <div class="inbox_sty">
            ...
            <div v-if="item.type == 1" class="inbox_sty">
              <span>{{ $t('set') + $t('card') }}：</span>
              <el-button
                v-if="!item.awardObject.couponName"
                size="mini"
                style="margin:0 10px;"
                type="primary"
                @click="chooseCard(index, 1)"
                >{{ $t('set') + $t('card') }}</el-button
              >
              <el-tag
                v-if="item.awardObject.couponName"
                style="margin:0 10px;"
                ...
                >{{ item.awardObject.couponName }}</el-tag
              >
            </div>
            <div v-if="item.type == 1" class="inbox_sty">
              <span>{{ $t('q') }}：</span>
              <el-input
                v-model="item.awardObject.quantity"
                style="width:50%;margin:0 10px;"
                type="number"
                min="0"
                clearable
            ...
            <div v-if="item.type == 2" class="inbox_sty">
              <span>{{ $t('reward') + $t('integral') }}:</span>
              <el-input
                v-model="item.awardObject.num"
                :placeholder="$t('p')"
                style="width:50%;margin:0 10px;"
                type="number"
                min="0"
          ...
          <el-button type="primary" size="mini" @click="addNewPrize(1)">{{
            $t('add')
          }}</el-button>
        </el-col>
      </el-form-item>
      ...
      <el-form-item :label="$t('brand.CumulativeSignInReward')">
        <el-col :span="10">
          <el-checkbox
            v-model="subInfo.openCumulativeAttendance"
            @change="iscumulative"
            >{{ $t('open') }}</el-checkbox
          >
        </el-col>
      ...
      <el-form-item v-if="subInfo.openCumulativeAttendance">
        <el-col :span="13">
          <div
            v-for="(item, index) in subInfo.cumulativeAttendanceAward"
            :key="index"
            class="box"
          >
            <div class="inbox_sty">
          ...
          <el-button type="primary" size="mini" @click="addNewPrize(2)">{{
            $t('add')
          }}</el-button>
        </el-col>
      </el-form-item>
    ...
    >
      ...
      >
        <el-table-column :label="$t('choose')">
          <template slot-scope="scope">
            <el-radio v-model="radioNumber" :label="scope.row.id">{{
              $t('set')
            }}</el-radio>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('card') + $t('validityPeriod')">
          <template slot-scope="scope">
            <span v-if="scope.row.beginTimestamp" style="margin-left: 10px"
              >{{ scope.row.beginTimestamp }}至{{
                scope.row.endTimestamp
              }}</span
            >
            <span v-if="!scope.row.beginTimestamp" style="margin-left: 10px">
              领取后
              <span v-if="scope.row.fixedBeginTerm">{{
                scope.row.fixedBeginTerm
              }}</span>
              <span v-if="!scope.row.fixedBeginTerm">当</span>
              天生效,有效天数{{ scope.row.fixedTerm }}{{ $t('d') }}
            </span>
          </template>
        </el-table-column>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" @click="confirmCard">{{
          $t('confirm')
        }}</el-button>
...
<script>
import { CardList, CreateActivities } from '@/api/marketing/signin'
...
export default {
  ...
  methods: {
        ...
        if (res.code == 0) {
          if (res.data.couponList) {
            ...
            res.data.couponList.map(res => {
              res.beginTimestamp = res.beginTimestamp
                ? formatDate(new Date(res.beginTimestamp * 1000), 'yyyy-MM-dd')
                : res.beginTimestamp
              res.endTimestamp = res.endTimestamp
                ? formatDate(new Date(res.endTimestamp * 1000), 'yyyy-MM-dd')
                : res.endTimestamp
            })
    ...
    empty(index, i) {
      // 单选框 卡券 积分(连续签到)
      if (i == 1) {
        this.subInfo.continuousAttendanceAward[index].awardObject.num = null
        this.subInfo.continuousAttendanceAward[index].awardObject.quantity = ''
        this.subInfo.continuousAttendanceAward[index].awardObject.id = null
        this.subInfo.continuousAttendanceAward[index].awardObject.couponName =
          ''
      } else {
        this.subInfo.cumulativeAttendanceAward[index].awardObject.num = null
        this.subInfo.cumulativeAttendanceAward[index].awardObject.quantity = ''
        this.subInfo.cumulativeAttendanceAward[index].awardObject.id = null
    ...
    deleteSign(index, i) {
      ...
      if (i == 1) {
        ...
        if (this.subInfo.continuousAttendanceAward.length == 0) {
          this.$set(this.subInfo, 'openContinuousAttendance', false)
        }
      } else {
        this.subInfo.cumulativeAttendanceAward.splice(index, 1)
        if (this.subInfo.cumulativeAttendanceAward.length == 0) {
          this.$set(this.subInfo, 'openCumulativeAttendance', false)
        }
      }
    },
    imageSuccessCBK(e) {
      // 上传图片
      console.log('wwwwwwwwwwwwweeeeeeee', e)
      this.subInfo.backgroundUrl = e[0].url.url
      console.log(this.subInfo.backgroundUrl)
    },
    deletePageImg() {
    ...
    submitForm(formName) {
      ...
      this.subInfo.international = this.ruleForm.international
      this.subInfo.description = this.ruleForm.description
      this.subInfo.beginTime = this.ruleForm.date[0] + ' ' + '00:00:00'
      ...
      console.log('121212121222', this.subInfo, this.ruleForm)
    ...
    createActivitiesInterface(obj) {
      ...
      // 创建活动接口
      obj.international = JSON.stringify(this.ruleForm.international)
      CreateActivities(obj).then(res => {
        if (res.code == 0) {
          console.log('创建活动接口', res)
          this.open('成功', '新增成功!', 'success')
          this.resetForm()
        }
      })
    },
