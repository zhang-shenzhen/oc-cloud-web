<template>
  <div>
    ...
    <div :style="{'font-size': '12px','color':color100,'margin-top': '10px'}" v-if='$route.query.OTP != 3'>
      <div> 概率是和需为100%</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    inputProbability() {
      ...
      console.log('inputProbability', d.probability)
        sunProbability += parseFloat(d.probability)
      })
      console.log('inputProbability', sunProbability)
      if (sunProbability == 100) {
        console.log('sunProbability == 100',this.tableData)
