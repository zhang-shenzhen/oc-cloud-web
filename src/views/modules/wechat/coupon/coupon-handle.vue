<template>
  <d2-container class="page">
    <h3 class="list-title">{{ $t('wechat.coupon.Add') }}</h3>
    <el-form
      ref="couponform"
      label-position="left"
      :rules="Rules"
      ...
      <div class="box">
        ...
        <el-form-item :label="$t('coupons.coverPhoto')">
          <div class="img-upload">
            ...
            <!-- <editorImage v-if="type == 'update'" urltype="wechat"  type="custom"  @successCBK="imageSuccessCoverImage"/> -->
          </div>
          <div style="clear:both" />
          <el-alert
            :closable="false"
        ...
        >
          <el-row>
            <el-col :span="24">
              ...
              <br>
              <el-radio-group v-model="form.codeType" :disabled="type == 'look'">
  ...
  data() {
    return {
      ...
      Rules: {
        ...
        notice: [{ required: true, message: '请输入操作提示', trigger: 'blur' }]
      },
      pickerOptions: {
        disabledDate: time => {
          // 设置可选择的日期为今天之后的一个月内
    ...
    getcouponInfo() {
      CouponInfo({ couponId: this.$route.query.id }).then(res => {
        if (res.code == 0) {
          ...
          // 有效期
          if (
            Info.couponInfo.baseInfo.date_info.type ==
            'DATE_TYPE_FIX_TIME_RANGE'
          ) {
            this.form.Timestamp = '1'
            if (Info.couponInfo.baseInfo.date_info.begin_timestamp) {
              this.form.validity.push(
                new Date(
                  Info.couponInfo.baseInfo.date_info.begin_timestamp * 1000
                )
              )
              this.form.validity.push(
                new Date(
                  Info.couponInfo.baseInfo.date_info.end_timestamp * 1000
                )
              )
            }
          } else if (
            Info.couponInfo.baseInfo.date_info.type == 'DATE_TYPE_FIX_TERM'
          ) {
            this.form.Timestamp = '2'
            this.form.fixedBeginTerm =
              Info.couponInfo.baseInfo.date_info.fixed_begin_term
          ...
          this.form.acceptCategory =
            Info.couponInfo.advancedInfo.use_condition.accept_category // 适用商品
          this.form.rejectCategory =
            Info.couponInfo.advancedInfo.use_condition.reject_category // 不适用商品

          this.form.canUseWithOtherDiscount =
            Info.couponInfo.advancedInfo.use_condition.can_use_with_other_discount // 不可以与其他类型共享门槛 ，填写false时系统将在使用须知里 拼写“不可与其他优惠共享”， 填写true时系统将在使用须知里 拼写“可与其他优惠共享”， 默认为true

          // 封面图片
          if (Info.couponInfo.advancedInfo.abstract.icon_url_list.length > 0) {
            this.form.coverImage =
              Info.couponInfo.advancedInfo.abstract.icon_url_list[0]
          }
          ...
          this.form.serviceStore =
            Info.couponInfo.advancedInfo.business_service
          ...
          // 门店设置
          this.form.useAllLocations =
            Info.couponInfo.baseInfo.use_all_locations

          if (!this.form.useAllLocations) {
            this.form.locationIdList =
              Info.couponInfo.baseInfo.location_id_list
          }
    ...
    setSendData() {
      const cardDataInfo = {
        ...
        couponInfo: {
          ...
          location_id_list: this.form.useAllLocations
            ? null
            : this.form.locationIdList,
          center_title: null,
          ...
          get_limit: JSON.stringify(this.form.getLimit),
