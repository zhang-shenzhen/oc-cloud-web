  ...
  <d2-container class="page">
    ...
    <el-form ref="ruleForm"
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   @click="resetForm('ruleForm')">立即发送</el-button>
      </el-form-item>
    </el-form>
  </d2-container>
</template>
...
import {
  ...
  DetailMessage,
  TagList
} from "@/api/wechat/groupMessage";

import {
  surbanajurong_SaveMessage,
  surbanajurong_SendMessage,
  surbanajurong_DetailMessage,
  ...
  created () {
    if (this.$route.query.projectName == "product") {
      ...
      this.API = {
        ...
        DetailMessage: surbanajurong_DetailMessage,
        TagList: surbanajurong_TagList
      };
    }
    this.tableList();
    this.getDetailMessage(this.$route.query.id);
  },
  mounted () { },
  beforeDestroy () { },
  methods: {
    ...
    getDetailMessage (id) {
      // 获取详情
      this.API.DetailMessage({}, id).then(res => {
        if (res.code == 0) {
          console.log("获取详情", res);
          this.groupMessage.id = res.data.id;
          this.groupMessage.tag_id = res.data.tagId;
          this.groupMessage.msg_info = res.data.msgInfo;
          this.groupMessage.msg_type = res.data.msgType;
          this.groupMessage.is_to_all = res.data.isToAll;
          this.ruleForm.tagtype = res.data.isToAll ? "1" : "2";
          this.ruleForm.tagId = res.data.tagId;
          this.getShowImg(this.groupMessage.msg_info);
        }
      });
    },
