  ...
  <d2-container class="page">
    ...
    <el-row>
      ...
      <el-col v-if="trigger.event.type==4"
              :span="20"
              :offset="2">
        <!-- <el-radio-group v-model="mode"@change="newsLabel()"> -->
        <el-radio-group v-model="trigger.event.data[0].mode"
                        @change="newsLabel()">
      ...
      <el-col v-if="trigger.event.type==4&&trigger.event.data[0].mode!=11"
        ...
        <div v-for="(item,index) in trigger.event.data"
          ...
          <el-select v-model="item.mode"
                     clearable
                     placeholder="请选择">
            <el-option v-for="i in newsSelect"
                       :key="i.value"
                       :label="i.label"
                       :value="i.value" />
          </el-select>
    ...
    <el-row>
      ...
      <el-col :span="20">
        <el-select v-model="trigger.operation.operationItems[0].type"
          ...
          <el-option label="修改会员信息"
                     :value="15" />
          <el-option v-if="trigger.event.type==0 || trigger.event.type == 1 || trigger.event.type == 3 || trigger.event.type == 4"
                     label="发送消息"
                     :value="1"></el-option>
          <el-option v-if="trigger.event.type==0 || trigger.event.type == 1 || trigger.event.type == 3 || trigger.event.type == 4"
                     label="图文"
                     :value="6"></el-option>
          <el-option v-if="trigger.event.type==6 || trigger.event.type==7"
                     label="模板消息"
                     :value="11"></el-option>
      ...
      <div v-if="trigger.operation.operationItems[0].type==15">
        <el-col :span="2"
                style="margin-top:10px;">选择字段</el-col>
        <el-col :span="20">
      ...
      <!-- 选择字段 v = 性别||是否为会员时 -->
      <div v-if="trigger.operation.operationItems[0].value[0].field=='sex'&&trigger.operation.operationItems[0].type==15">
        <el-col :span="2"
                style="margin-top:10px;">性别</el-col>
        <el-col :span="20">
      ...
      <div v-if="trigger.operation.operationItems[0].value[0].field=='memberOrNot'&&trigger.operation.operationItems[0].type==15">
        <el-col :span="2"
                style="margin-top:10px;">是否为会员</el-col>
        <el-col :span="20">
          <el-select v-model="trigger.operation.operationItems[0].value[0].value"
                     suffix-icon="el-icon-date"
                     style="width:100%"
...
export default {
  ...
  methods: {
    detailsInterface (id) {
      ...
      Details({}, id).then(res => {
        if (res.code == 0) {
          ...
          if (this.trigger.operation.extraOperation != null) {
            this.tageShow = true
            this.trigger.operation.extraOperation.value.map(res => {
              this.tagNameList.push(res.id)
            })
          }
          if (!this.trigger.operation.extraOperation.value.length) {
            this.tageShow = false;
          }
          // this.echoDisplay(this.trigger.event.type);
          if (this.trigger.event.type == 8) {
            const msg = JSON.parse(this.trigger.event.data[0].value)
            this.lstime = msg.type
            console.log('111', msg)
            if (this.lstime == 1) {
              this.time.dateTime = msg.data
              this.time.weekTime = msg.hour + ':' + msg.minute
            ...
            this.trigger.event.data[0].mode = this.trigger.event.data[0].mode + ''
            // this.trigger.event.data[0].value = this.trigger.event.data[0].value;
            // 出现回显下拉框显示id问题
            this.trigger.event.data[0].value = parseInt(this.trigger.event.data[0].value)
          }
          console.log('我是看看你是不是选择了图文', this.trigger.operation.operationItems, this.trigger.operation.operationItems[0].type)
          if (this.trigger.operation.operationItems[0].type == 6) {
            console.log('123333333', this.trigger.operation.operationItems)
            const showDataObj = {
              type: this.trigger.operation.operationItems[0].value[0].wechatMaterialNewsItemList[0].type,
              value: [
                {
    ...
    getValue (e) {
      ...
      console.log(e, this.trigger.operation.operationItems, '我看看你传进去的是谁')
    },
    choosenable () {
      // 是否打标签
