<template>
  <d2-container class="page">
    <h3 class="list-title">{{ $t('wechat.trigger.addTrigger') }}</h3>
    <el-form ref="trigger"
             :model="trigger"
             :rules="rules"
             size="mini"
             label-width="160px"
             class="demo-ruleForm">
      <el-form-item :label="$t('member.filter.nickName')"
                    prop="name">
        <el-col :span="9">
      ...
      <!-- 触发条件 -->
      <el-form-item :label="$t('wechat.trigger.condition')">
        <el-col :span="9">
      ...
      <el-form-item>
        <el-button type="primary"
                   size="mini"
                   @click="submitForm('trigger')">立即创建</el-button>
        <el-button size="mini"
...
export default {
  ...
  methods: {
    ...
    dealDate (obj) {
      ...
      console.log('这里面是处理的', type, value)
      if (type == 1) {
        value.localUrl = "";
    ...
    submitForm (formName) {
      ...
      console.log(this.trigger, "我要看看你是变速出来了");
