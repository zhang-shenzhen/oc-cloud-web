    ...
    <el-form ref="param"
      ...
      <el-form-item>
        <el-button type="primary"
                   @click="submitForm('param')">确认</el-button>
...
import {
  ...
  surbanajurong_defaultUpdate,
  surbanajurong_FocusUpdate
...
export default {
  ...
  created () {
    if (this.$route.query.projectName == "product") {
      this.API = {
        ...
        DefaultUpdate: ReplyUpdate,
        FocusUpdate: ReplyUpdate
      };
    } else if (this.$route.query.projectName == "surbanajurong") {
      this.API = {
        DetailsMessage: surbanajurong_DetailsMessage,
        DefaultUpdate: surbanajurong_defaultUpdate,
        FocusUpdate: surbanajurong_FocusUpdate
      };
    }
  ...
  methods: {
    ...
    judgeSelection () {
      ...
      if (this.ReplyList[0].getVal.action == "文本") {
        ...
        if (this.ReplyList[0].getVal.number == 1) {
          ...
          this.operationItems[0].value[0].localUrl = this.ReplyList[0].getVal.advancedNewsItemList[0].picUrl;
          this.operationItems[0].value[0].wechatMaterialNewsItemList = this.ReplyList[0].getVal.advancedNewsItemList;
        }
    ...
    replyUpdateInterface (obj) {
      ...
      console.log(obj);
      if (this.replyObj.createType == 1) {
        this.API.DefaultUpdate(obj).then(res => {
          if (res.code == 0) {
        ...
        this.API.FocusUpdate(obj).then(res => {
          if (res.code == 0) {
            this.open("成功", "编辑成功", "success");
            console.log("编辑接口", res);
            this.cancel();
    ...
    cancel () {
      // let name;
      // if (this.replyObj.createType == 1) {
      //   name = "wechat-reply-defaultReply";
      // } else {
      //   name = "wechat-reply-foucusReply";
      // }
      // this.$router.push({
      //   name: name
      // });
