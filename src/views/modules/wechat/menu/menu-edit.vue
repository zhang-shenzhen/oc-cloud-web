    ...
    <el-row>
      <el-col :span="8"
        ...
        <!-- <div class="bg-purple-dark">
          <div class="one-level">
            <div v-for="(item,index) in menuContent.button" :key="index" class="level two-level">
              <div class="two-level-title" @click="oneLevel(index)">{{ item.name }}</div>
              <div v-if="menu.menuIndex == index" class="two-level-wrap">
                <div class="level two-level-add" @click="addTwoMenuTitle(index)">+</div>
                <div
                  v-for="(itm,ind) in item.sub_button"
                  :key="ind"
                  class="level two-level-title"
                  @click="subMenu(index,ind)"
                >{{ itm.name }}</div>
              </div>
            </div>
            <div v-if="menuContent.button.length!=3" class="level" @click="addOneMenu()">+</div>
          </div>
        </div> -->
      </el-col>
      <el-col :span="10"
        ...
        <div class="menu-group-name">
          ...
          <div v-if="menu.isOrNo"
               @click="deleteMenu()">{{$t('wechat.menu.des14')}}</div>
        </div>
        <el-alert class="menu-alert"
                  v-if="!menu.isOneLevelNoSub && !menu.isSubMenu"
  ...
  created () {
    if (this.$route.query.projectName == "product") {
      ...
      this.API = {
        ...
        Details: surbanajurong_Details
      };
    }

    console.log("123456543", this.$route.query);
    // 获取路由跳转的参数↓
    this.menu.isOrNo = this.$route.query.isOrNo;
    ...
    this.menuContent.versionId = this.$route.query.currentVersionId;
    this.menuContent.individuation = this.$route.query.individuation;
    this.tagNoPageInterface();
    this.menuDetailInterface(this.menuContent.versionId);
  },
  mounted () { },
  beforeDestroy () { },
  methods: {
    menuDetailInterface (id) {
      ...
      this.API.MenuDetail({ id: id }).then(res => {
        if (res.code == 0) {
          ...
          if (echo.wechatMenuMatchrule) {
            this.tagMenu(echo.wechatMenuMatchrule.tagId);
          }
          this.echomenu(echo.wechatMenuList); // 拿到wechatMenuList里的参数
        }
      });
    },
    tagMenu (tagId) {
    ...
    echomenu (array) {
      const submenuList = [];
      console.log(array, '这些时我要做回显用的要处理的参数')
      array.map((res, index) => {
        console.log("232323", res.operation.operationItems);
        // if (!res.operation.operationItems) {
        //   res.type = "click";
        // }
        if (res.subMenuList.length != 0) {
          res.type = "click";
        }
        console.log(res, '这个是我拿到的数据===')
        submenuList.push({
          type: res.type,
          name: res.name,
          url: res.url,
          ...
          triggerOperationItem: {
            type: res.operation.operationItems ? res.operation.operationItems[0].type : "",
            value: res.operation.operationItems ? res.operation.operationItems[0].value : []
          },
          sub_button: []
        });
        if (res.subMenuList.length != 0) {
          res.subMenuList.map(item => {
            submenuList[index].sub_button.push({
              type: item.type,
              name: item.name,
              url: item.url,
              appid: item.appid,
              ...
              triggerOperationItem: {
                type: item.operation.operationItems ? item.operation.operationItems[0].type : "",
                value: item.operation.operationItems ? item.operation.operationItems[0].value : []
              }
            });
          });
        }
      });
      this.menuContent.button = submenuList;
      if (this.menuContent.button[0].sub_button.length != 0) {
        this.menu.isOneLevelNoSub = false;
      }
      this.echoMenu(this.menuContent.button[0]);

      console.log(submenuList, this.menuContent.button, "11112222");
    },
    tagNoPageInterface () {
      // 获取标签接口
      this.API.TagNoPage({
    ...
    addTwoMenuTitle (i) {
      ...
      // console.log('1234567898765434567', this.menu.subMenuIndex,this.menuContent.button[i].sub_button.length);
    ...
    echoMenu (obj) {
      ...
      if (obj.triggerOperationItem.type == 6 || obj.triggerOperationItem.type == 5) {
        obj.type = "click";
        showData.type = obj.triggerOperationItem.type;
        showData.number = obj.triggerOperationItem.type;
        showData.value[0].id = obj.triggerOperationItem.value[0].news;
        showData.value[0].localUrl = obj.triggerOperationItem.value[0].echoImg;
        if (obj.triggerOperationItem.type == 6) {
          showData.value[0].wechatMaterialNewsItemList = [
            {
              id: obj.triggerOperationItem.value[0].news,
              localThumbUrl: obj.triggerOperationItem.value[0].echoImg,
            }
          ];
        }
        if (obj.triggerOperationItem.type == 5) {
          showData.value[0].wechatMaterialNewsItemList = [
            {
              id: obj.triggerOperationItem.value[0].news,
              picUrl: obj.triggerOperationItem.value[0].echoImg,
            }
          ];
        }
      ...
      console.log('原来要在这里处理插件的问题', option)
    ...
    subMenu (i, num) {
      ...
      console.log(this.menuContent.button[i].sub_button[num], '我用来回显的数据')
    },
    ...
    judgeAssignment (obj) {
      // 判断赋值
      // this.menu.menuIndex 第几个父级菜单
      // this.menu.subMenuIndex 第几个子菜单
      console.log(obj, this.menu.subMenuIndex, "obj插件1234567890");
      if (obj.action == "文本") {
        if (this.menu.subMenuIndex == -1) {
          this.menuContent.button[this.menu.menuIndex].triggerOperationItem = {
            type: 1,
      ...
      if (obj.action == "图文") {
        ...
        if (obj.number == 2 && this.menu.subMenuIndex == -1) { // 高级图文
          this.menuContent.button[this.menu.menuIndex].triggerOperationItem = {
            type: 5,
            value: [{
              news: obj.id,
