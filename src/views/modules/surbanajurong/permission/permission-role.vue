        ...
        <el-form>
          <el-form-item class="search-box-item" label style="display:flex;">
            <div style="display:flex;line-height: 0;margin-top: 5px;">
              <el-input v-model="searchName" size="mini" :placeholder="$t('role.roleName')" />
              <el-button
              ...
              >{{$t('search')}}</el-button>
              <el-button
                v-if="$Buttonpermissions('surbanajurong.permission.permission-role[add-role]')"
                size="mini"
                type="primary"
                @click="isAddOrEdit()"
              >{{$t('add')}}</el-button>
      ...
      >
        ...
        <template v-slot:operating="operating">
          <el-button
            type="text"
            size="mini"
            v-if="$Buttonpermissions('surbanajurong.permission.permission-role[update-role]')"
            @click="isAddOrEdit(operating.operating)"
          >{{$t('update')}}</el-button>
          <el-button
            type="text"
            size="mini"
            style="color:#f56c6c"
            v-if="$Buttonpermissions('surbanajurong.permission.permission-role[del-role]')"
            @click="deleteAlert(operating.operating)"
          >{{$t('delete')}}</el-button>
        </template>
      </table-mob>
      ...
      <div class="sysRoleInfoData">
        <el-form ref="tree" :model="dataForm" size="mini" label-width="80px">
          <el-form-item :label="$t('role.roleName')">
            <el-input :placeholder="$t('role.roleName')" v-model="dataForm.name" prop="name" />
          </el-form-item>
          <el-form-item :label="$t('remark')">
            <el-input :placeholder="$t('remark')" v-model="dataForm.note" prop="note" size="mini" />
          </el-form-item>
          <el-form-item :label="$t('role.roleTree')">
            <el-tree
              ref="menuListTree"
              :data="treeMenuList"
              :props="{ label: 'name', children: 'childList' }"
          ...
          <el-form-item>
            <el-button size="mini" type="primary" @click="addOrEdit">{{$t('confirm')}}</el-button>
            <el-button size="mini" @click="dialogDriverInfoVisible = false">{{$t('cancel')}}</el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-dialog>
...
export default {
  ...
  computed: {},
  created() {
    this.language = cookieGet("language") || "zh-CN";
    this.getList(1);
    this.initTab();
  ...
  methods: {
    ...
    getPermissionUser() {
      ...
      const userId = JSON.parse(localStorage.getItem(`userInfo-${process.env.VUE_APP_VERSION}`)).id;
      ...
      if (arr.length > 0) {
        for (const i in arr) {
          if (this.language == "zh-CN") {
            ...
            arr[i].name = JSON.parse(arr[i].name).en_US;
          }
    ...
    getList(page) {
      ...
      if(!this.$isNull(this.searchName)){
